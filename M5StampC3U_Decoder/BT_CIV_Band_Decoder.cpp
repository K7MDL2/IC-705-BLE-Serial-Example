...
void bt_loop(void) {
  if (BT_enabled && !btConnected) {
    draw_new_screen();
    DPRINTLNF("BT Loop");
    Serial.print(F("   Using bd_address:"));  // print default bd_address
    for (int z = 0; z < 6; z++)
...
void display_Time(uint8_t _UTC, bool _force) {
  ...
  if ((millis() >= time_last_disp_UTC + POLL_RADIO_UTC) || _force) {
    ...
    #ifdef M5STAMPC3U
    //sprintf(temp_t, "%02d/%02d/%04d", month(), day(), year());
    //DPRINTF("Radio Date is "); DPRINT(temp_t);
    //sprintf(temp_t, "%02d:%02d:%02d", hour(), minute(), second());
    //DPRINTF(" and time is "); DPRINTLN(temp_t);
    #else
...
void display_Freq(uint64_t _freq, bool _force) {
  ...
  if ((_freq != _prev_freq && _freq != 0) || _force) {
    ...
    Serial.printf("VFOA: %13sMHz - Band: %s   Grid:%s\n", formatVFO(_freq), bands[band].band_name, Grid_Square);
    #endif
    #ifndef M5STAMPC3U
...
void display_Grid(char _grid[10], bool _force) {
  static char _last_grid[10] = {};
  int x = 8;
  int y = 184;
  int font_sz = 4;
