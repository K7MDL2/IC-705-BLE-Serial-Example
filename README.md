...
Operated portable (K7MDL/P) in Sept 2025 VHF contest.  Had reports of RFI on audio on 222.  After the contest I tracked it down to the 10MHz ref coax picking up 222 RF when the front panel was installed and touching the 10Mhz cable. A small snap-on ferrite cleared it up. 28, 50, 144 and 1296 bands were clear (144 IF).   All worked as it should except for the preamp not getting reliably restored on band switches.   Pictures from Mt Constitution at 1400ft road turnout looking south over Puget Sound (in Washington).
...
May 23, 2025 - Fixed RFI issue while bench testing transverter.  Transmitting on a nearby HF radio at 100W on 20M caused the transverter box to toggle bands every second or 2.  Added 0.1uF caps to the C3U band and PTT input pins. There are already caps on the M5Core controller end of the cable.  I also coiled some of the control cable.  I am using a M5Stack PLCC module with a ULN2803A for buffering on the M5Core controller end. Also fixed a bootup situation where all 3 transverters were powered on briefly.  Added software debounce then removed it after the caps alone solved the RFI problem.  Do not want to add any PTT delay if I do not need to.

Here is my 3-Band transverter mounted on a plywood base along with a PowerPole fuse distribution box and a Toptec 180W 2M amp (bottom with fan) and a Toptec 80W 430Mhz amp stacked to save space.  This slides under my rear truck bench seat where I have 12V@50A from my truck camper which has a 100Ah LiFePO4 battery, 160W solar panel, and 50A DC-DC charger from the truck alternator. I used 6AWG wire throughout.  There is also a 50Ah LiFePO4 battery mainly used for the microwave gear on a tripod but can swap with the 100Ah battery for backup power or to recharge it in the field.
...
On the back panel, under the IF T/R board, I sanded the paint off and mounted a 50ohm termination resistor.  IT wil take up to 10W for the 705.  and reduces it to an adjustable TX drive around -15 to +5dBm.  There is some loss in the SP6T IF switch to make up for. The 3 Xvtrs require around 0-5dBm input, 10dBm absolute max.  I am running the radio at about 30% TX level, or 3W out.  This give me a bit of range to reduce power.  The 1-2W output from the 1296 internal amp is enough to drive the external SG-Labs 25W PA+LAN module.   903Mhz is about 46dBm, and 39dBm for the barefoot 222Mhz Xvtr.  

To finish this project and use it for my Rover truck this summer (2025), I need to decide on a 28V solution for the 50W 903 amp, and see if I can scrape up a 50W-ish 222 amp.  The imported 50W 222 RF deck I tested is unusable for linear work due to really terrible harmonics.  28V switching is set up on the relays, just waiting for a 28V source good up to 5A.  I also need to settle on a good combo of logging and WSJT-X app that can work together smoothik with a 705 that has 3 extra bands.   There are info exchange issues between AC Log/VHF Contest and WSJT-X, they are different depending on who has control of the CI-V port. Those issue are unrelated to the Xvtr and radio.
